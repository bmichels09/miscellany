function setTooltip(spanId, text) {
    function mouseOver(){
...
function drawTable(data) {
    ...
    resultLabels.append("th").attr("id", "resultLabelGrowthIndex").html("Growth Index");
    resultLabels.append("th").attr("id", "resultLabelMarketCap").html("Market Cap");

    var growth21DayTimestamps = data.map(x => {
        try {
            if (x.growth21DayStartDate === "") {return 0;}
            else {
                return (new Date(Number(x.growth21DayStartDate.split('-')[0]), Number(x.growth21DayStartDate.split('-')[1]) - 1, Number(x.growth21DayStartDate.split('-')[2]), 0, 0, 0, 0)).getTime();
            }
        }
        catch {
            return;
    ...
    data.forEach(x => {
        ...
        if (x.growth21DayStartDate !== correctDate) {
            tooltip = tooltip + `<br><span style="color: rgb(240, 135, 132)"><b>!</b> Data may be inaccurate due to missing days</span>`;
            growthIndexHtml = `<span style="color: rgb(240, 135, 132)"><b>${Intl.NumberFormat('en-US', { style: 'decimal', minimumFractionDigits: 2, maximumFractionDigits: 2 }).format(x.marketIndIndex)}</b></span>`;
            return;
        }
        else {
        ...
        resultRow.append("td")
            .attr("id", `resultDataGrowthIndex_${x.ticker}`)
            .html(growthIndexHtml);
        resultRow.append("td")
        ...
        setTooltip(`resultDataGrowthIndex_${x.ticker}`, tooltip);    
    });
}
...
(async () => {
                ...
                keys.forEach(key => {
                    try {
                        ...
                        collectedData.push({
                            ...
                            growth21Day: "",
                            avg21Day: "",
                            growth30Day: chartData.day30ChangePercent,
                            growth3DayStartDate: "",
                            growth3DayEndDate: "",
                            growth21DayStartDate: "",
                            growth21DayEndDate: "",
                            avg21DayStartDate: "",
                            avg21DayEndDate: ""
                        });
    ...
    collectedData.sort((a, b) => {
        if (Math.abs(a.growth30Day + 0.3625) < Math.abs(b.growth30Day + 0.3625)) {return -1;}
        else if (Math.abs(a.growth30Day + 0.3625) > Math.abs(b.growth30Day + 0.3625)) {return 1;}
        else if (a.ticker > b.ticker) {return 1;}
        else {return -1;}
    });

    for (var i = 0; i < calculate; i++) {
        if (i >= collectedData.length) {break;}

        if (i % 100 === 0) {batches.push([]);}

        batches[Math.floor(parseFloat(i) / 100)].push(collectedData[i].ticker);
    }
    ...
    for (var i = 0; i < batches.length; i++) {
        ...
        let timeoutEnd = (new Date()).getTime() + 12;
        do {} while ((new Date()).getTime() < timeoutEnd)
        let url = `https://cloud.iexapis.com/stable/stock/market/batch?symbols=${symbols}&types=chart&range=3m&chartCloseOnly=true&includeToday=true&token=${stocksKey}`;
        const done = await axios.get(url)
            .then(response => {
                let keys = Object.keys(response.data);
                keys.forEach(key => {
                    try {
                        ...
                        let dataIndex = collectedData.findIndex(x => x.ticker === key);
                        ...
                        if (chartData.length > 3) {
                            let price3DaysAgo = chartData[chartData.length - 4].close;
                            let priceNow = chartData[chartData.length - 1].close;
                            growth3Day = parseFloat(priceNow) / parseFloat(price3DaysAgo) - 1;
                            collectedData[dataIndex].growth3Day = growth3Day;
                            collectedData[dataIndex].growth3DayStartDate = chartData[chartData.length - 4].date;
                            collectedData[dataIndex].growth3DayEndDate = chartData[chartData.length - 1].date;
                        }

                        if (chartData.length > 21) {
                            let price21DaysAgo = chartData[chartData.length - 22].close;
                            let priceNow = chartData[chartData.length - 1].close;
                            growth21Day = parseFloat(priceNow) / parseFloat(price21DaysAgo) - 1;
                            collectedData[dataIndex].growth21Day = growth21Day;
                            collectedData[dataIndex].growth21DayStartDate = chartData[chartData.length - 22].date;
                            collectedData[dataIndex].growth21DayEndDate = chartData[chartData.length - 1].date;
                        }

                        if (chartData.length >= 21) {
                            let last21Days = chartData.slice(-21);
                            let sum21Day = last21Days.map(x => parseFloat(x.close)).reduce((a, b) => a + b);
                            let avg21Day = sum21Day / 21;
                            vsAvg21Day = parseFloat(last21Days[last21Days.length - 1].close) / avg21Day - 1;
                            collectedData[dataIndex].avg21Day = vsAvg21Day;
                            collectedData[dataIndex].avg21DayStartDate = last21Days[0].date;
                            collectedData[dataIndex].avg21DayEndDate = last21Days[last21Days.length - 1].date;
                        }

                        if (growth3Day !== undefined && growth21Day !== undefined && vsAvg21Day !== undefined) {
                            let subtotal1 = Math.pow(Math.abs(growth3Day + 0.165) / 0.085, 3);
                            let subtotal2 = Math.pow(Math.abs(growth21Day + 0.3625) / 0.1575, 3);
                            let subtotal3 = Math.pow(Math.abs(vsAvg21Day + 0.261) / 0.149, 3);
                            let subtotal4 = Math.pow(subtotal1 + subtotal2 + subtotal3, 1 / parseFloat(3));
                            collectedData[dataIndex].marketIndIndex = 10 - subtotal4;
                        }
                    }
                    catch (error) {
    ...
    collectedData.sort((a, b) => {
        if (a.marketIndIndex !== "" && b.marketIndIndex === "") {return -1;}
        else if (b.marketIndIndex !== "" && a.marketIndIndex === "") {return 1;}
        else if (a.marketIndIndex > b.marketIndIndex) {return -1;}
        else if (a.marketIndIndex < b.marketIndIndex) {return 1;}
        else {return 0;}
    });
